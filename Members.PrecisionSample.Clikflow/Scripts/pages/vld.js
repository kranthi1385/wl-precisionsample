define(['app'], function (app) {
   ...
   function ($rootScope, $scope, $window, $timeout, httpService, getQueryParams, translationsLoadingService, $interval, $cookies) {
       ...
       $scope.DFIQ = function () {
           var uniquenessParam = Forensic.createUniquenessParam(uig);
           var geoParam = null;
           if (cc != '' && zip != '') {
               var geoParam = Forensic.createGeoParam(cc, zip);
           }
           Forensic.forensic(ug, successCallback, errorCallback, uniquenessParam, geoParam, null, null, "Reach Collective");
       }
       function successCallback (jsonData) {
           $scope.dfiqJson = JSON.stringify(jsonData);
       };
