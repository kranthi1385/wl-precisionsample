...
using Members.OpinionBar.Web.Utlis;
using System.Text;
using System.Net;
using System.IO;
using NLog;
    ...
    public class RgController : BaseController
    {
        ...
        public ActionResult Top10(Guid ug, Guid? uig, Guid? usg, int cid)
        {
            ...
            {                                                   //.. | ..//
                ...
                string referrer = System.Configuration.ConfigurationManager.AppSettings["AffilaitePixelUSHispanic"].ToString();//added on 03/15/2016 to fire pixels only for us hispanic affiliates
        ...
        [HttpPost]
        public void saverelevantinfo(string ug, int score, string rvid, int pfScore, string fpfScores, int isNew, int rchecker, int cid, string token, string passParam, string geoLonLat, string ipqsJson)
        {
            ...
            logger.Info("IPQS JSON|" + ipqsJson);
            UserManager oManager = new UserManager();
            ...
            com.imperium.verity.RespSvc client = new Members.OpinionBar.Web.com.imperium.verity.RespSvc();
            com.imperium.verity.ValidationResultsPlus9 vr = new Members.OpinionBar.Web.com.imperium.verity.ValidationResultsPlus9();
            com.imperium.verity.AuthHeader authentication = new Members.OpinionBar.Web.com.imperium.verity.AuthHeader();
            authentication.Username = System.Configuration.ConfigurationManager.AppSettings["VerityUserName"].ToString();
            authentication.Password = System.Configuration.ConfigurationManager.AppSettings["VerityPassword"].ToString();
            ...
            if (!string.IsNullOrEmpty(userVerityInfo)) //Implement of first Survey Click.
            {
                ...
                {
                    vr = client.ValidateDataPlus9(System.Configuration.ConfigurationManager.AppSettings["VerityClientId"].ToString(),
                    ...
                    if (vr.MatchResults.DOB.ToLower() == "no match" && System.Configuration.ConfigurationManager.AppSettings["VerityDOBON"].ToString() == "1")
                    {
                        verityScore = 2;
                        verityDOBFail = true;
                    }
            ...
            if (rchecker == 1) //Added to test Pixel Firing BY Excluding the Relevant Check :
            {
                ...
                oManager.ReleventUpdateForSDLOrWL(new Guid(ug), score, pfScore, fpfScores, rvid, cid, verityId, verityScore, geoCorrelationFlag, verityDOBFail, ipqsJson);
            }
            else
            {
