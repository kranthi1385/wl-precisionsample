...
namespace Members.PrecisionSample.Clikflow.Controllers
{
  ...
  string transid, string project, string sub, string tx_id, string frid, string lid, string sub_id)
        ...
        [HttpPost]
        public JsonResult RdjsonInsert(int userid, string uid, int cid, string uig, string ug, string dvtype, string sessionId = null)
        {
            string ipqsJson;
            ...
            logger.Info("Invitation GUID:" + uig + "User GUID:" + ug + "| IPQS JSON |" + ipqsJson);
            string clientIp = Request.ServerVariables["REMOTE_ADDR"].ToString();
            userObj.RedirectURL = oManager.RdjsonInsert(userid, uid, cid, uig, ug, ipqsJson, clientIp,dvtype,sessionId);
