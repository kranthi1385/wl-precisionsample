...
define(['app'], function (app) {
    app.register.controller('extController', ['$scope', '$rootScope', '$window', '$timeout', 'httpService', 'getQueryParams', '$interval',
   function ($scope, $rootScope, $window, $timeout, httpService, getQueryParams, $interval) {
       ...
       var hash = getQueryParams.getUrlVars()['hash'];
       if (hash == undefined) {
           hash = 0;
       }
       ...
       //                    $scope.token = result.results[0].token;
       //                    saveRid();
       //                }
       //            }
       //            else {
       ...
       function saveRid() {
           ...
           if (mid != undefined || pid != undefined) {
               httpService.postData("/e/ExtUpdate?qg=" + qg + "&mid=" + mid + '&pid=' + pid + '&rid=' + rid + '&source=' + source + '&subId=' + subId + '&isNew=' + isNew +
                          '&osId=' + osId + "&tscore=" + totalScore + '&frid=' + fedrespid + '&ecost=' + ecost + '&e_rm=' + e_rm + '&e_rl=' + e_rl + '&is_dupe=' + $scope.IsDupe +
                          '&external_member_id=' + $scope.ExternalMemberId + "&project_id=" + $scope.ProjectId + "&org_id=" + $scope.OrgId + "&external_member_guid=" + $scope.ExternalMemberGuid +
                           "&hash=" + hash).then(function (data, status) {
                               if (data != undefined && data != '') { //if we get question then fetch the question else redirect to next page
                                   //debugger;
                                   if (data.FedRedirectURl != '' && data.FedRedirectURl != null) {
                                       window.location.href = data.FedRedirectURl;
                                   }
                                           ...
                                           if ($scope.surveyResponse.RedirectUrl == "") {
                                               ...
                                               if (deviceMatchedCount == 1 && isMobile == 2) {
                                                   if ($scope.surveyResponse.IsStandalone) {
                                                       window.location.href = "https://e.opinionetwork.com/e/psr?usg=5CE933ED-9891-4CD7-8AC6-B529C58C6B55&uig=" + $scope.surveyResponse.ActualInvitationGuid + '&ug=' + ug + '&pid=' + prjId + '&cid=' + cid; //redirect to endpage 
                                                   }
                                                   else {
                                                       window.location.href = "www.opinionetwork.com/reg/home?ug=" + ug + "&cid=" + cid; //redirect to mobile survey page
                                                   }
                                               }
                                               ...
                                               if (deviceMatchedCount == 1 && isMobile == 3) {
                                                   ...
                                                   if ($scope.surveyResponse.CountyId == "231") {
                                                       if ($scope.surveyResponse.IsStandalone) { // check standalone partner
                                                           ...
                                                           {
                                                               window.location.href = "https://e.opinionetwork.com/e/psr?usg=50AD6CC9-9228-496F-B936-7D0E0973E60A&uig=" + $scope.surveyResponse.ActualInvitationGuid + '&ug=' + ug + '&pid=' + prjId + '&cid=' + cid; //redirect to endpage
                                                           }
                                                           showSms();
                                                       }
                                                       else {
                                               ...
                                               window.location.href = $scope.surveyResponse.RedirectUrl + '&pid=' + $scope.surveyResponse.ProjectId;
                                           }
                                       }
                                   }
                                   else {
                                       if ($scope.surveyResponse.ExternalPartnerID == 231) {
                                           window.location.href = "https://e.opinionetwork.com/e/psr?usg=F80A29B3-D4AD-42FD-82E5-53A6B47F9DB9&uig=" + $scope.surveyResponse.memberGuid; //redirect to endpage
                                       }
                                       if ($scope.surveyResponse.FedResponseID != "" && $scope.surveyResponse.FedResponseID != undefined) {
                                           window.location.href = "https://e.opinionetwork.com/e/psr?usg=F80A29B3-D4AD-42FD-82E5-53A6B47F9DB9&uig=" + $scope.surveyResponse.ActualInvitationGuid + '&ug=' + ug + '&pid=' + prjId + '&cid=' + cid; //redirect to endpage 
                                       }
                                       else {
