...
.controller('crController', ['$rootScope', '$scope', '$window', '$timeout', 'httpService', 'getQueryParams', 'lazyscriptLoader', '$interval',
   function ($rootScope, $scope, $window, $timeout, httpService, getQueryParams, lazyscriptLoader, $interval) {
       ...
       function saveRid(rvid, profileScore, score, fpfScore, isNew, rcheckR) {
           // post user responses
           if (rcheckR == '') {
               rcheckR = 0;
           }
           ...
           httpService.postData('/rg/saverelevantinfo?ug=' + ug + '&score=' + score + '&rvid=' + rvid + '&pfScore=' + profileScore + '&fpfScores=' + fpfScore + '&isNew=' + isNew + '&rchecker=' + rcheckR + '&cid=' + clientId + '&ipqsJson=' + $scope.IPQSJson).then(function (data, status) {
               $interval.cancel(poling);
               if (clientId == 383) {
                   window.location.href = '/hm/Home?ug=' + ug + '&lpage=' + lpage + '&confirm=f';
               }
               else if (clientId == 385) {
                   window.location.href = "/rg/top10?ug=" + ug + '&cid=' + clientId;
               }
               else if (score > 75 || score == -1) {
       ...
       var poling = "";
       $timeout(function () {
           poling = $interval(saveRid(rvid, profileScore, score, fpfScore, isNew, rcheckR), 3000)
