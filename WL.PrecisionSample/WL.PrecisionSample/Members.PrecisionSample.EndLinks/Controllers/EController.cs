...
namespace Members.PrecisionSample.EndLinks.Controllers
{
    public class EController : Controller
    {
            ...
            if (!string.IsNullOrEmpty(uig) && _uig != Guid.Empty)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else if (oSurveys.ExternalPartnerID == 210) //CINT
                        {
                    ...
                    if (objRecaptcha.HasPrescreenerQuestions)
                    {
                        ...
                        if (survyUrl.client_id == 7047 || survyUrl.client_id == 7159)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    survyUrl.ProjectUrl = "https://e.opinionetwork.com/e/psr?usg=F6A6B754-4CF8-41BB-B9AA-5B97C412B1F4&ug=" + ug + "&uig=" + objRecaptcha.UserInvitationGuid + "&marketplace_status=" + objInvite.ResultCode.ToString();
                                }
                                if (overQuota.Contains(objInvite.ResultCode))
                                {
                                    survyUrl.ProjectUrl = "https://e.opinionetwork.com/e/psr?usg=67B98BED-9C3F-42AE-BDD3-7E15F9C17F00&ug=" + ug + "&uig=" + objRecaptcha.UserInvitationGuid + "&marketplace_status=" + objInvite.ResultCode.ToString();
                                }
                                if (objInvite.ResultCode == 31)
                                {
                                    survyUrl.ProjectUrl = "https://e.opinionetwork.com/e/psr?usg=5CE933ED-9891-4CD7-8AC6-B529C58C6B55&ug=" + ug + "&uig=" + objRecaptcha.UserInvitationGuid + "&marketplace_status=" + objInvite.ResultCode.ToString();
                                }
                                if (objInvite.ResultCode == 38)
                                {
                                    survyUrl.ProjectUrl = "https://e.opinionetwork.com/e/psr?usg=664B50CB-E1E7-40CC-B2EB-A94E1D54228F&ug=" + ug + "&uig=" + objRecaptcha.UserInvitationGuid + "&marketplace_status=" + objInvite.ResultCode.ToString();
                                }
        ...
        public string CINTPostRequest(string postUrl, string LoginCredentials, string key)
        {
            ...
            LoginRequest.ContentType = "application/json; charset=UTF-8";
            LoginRequest.ContentLength = data.Length;
            LoginRequest.Headers.Add("X-Api-Key", key);
            Stream LoginRequestStream = LoginRequest.GetRequestStream();
            LoginRequestStream.Write(data, 0, data.Length);
            LoginRequestStream.Close();
