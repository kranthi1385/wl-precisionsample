...
namespace Members.NewOpinionBar.Web.Controllers
{
    public class RgController : BaseController
    {
        ...
        [HttpPost]
        public void saverelevantinfo(string ug, int score, string rvid, int pfScore, string fpfScores, int isNew, int rchecker, int cid, string token)
        {
            string RequestURL = string.Empty;
            RequestURL = System.Configuration.ConfigurationManager.AppSettings["ResearchDefender"].ToString();
            RequestURL = string.Format(RequestURL, ug, cid);
            ...
            com.imperium.verity.RespSvc client = new Members.NewOpinionBar.Web.com.imperium.verity.RespSvc();
            com.imperium.verity.ValidationResultsPlus9 vr = new Members.NewOpinionBar.Web.com.imperium.verity.ValidationResultsPlus9();
            com.imperium.verity.AuthHeader authentication = new Members.NewOpinionBar.Web.com.imperium.verity.AuthHeader();
            ...
            if (rchecker == 1) //Added to test Pixel Firing BY Excluding the Relevant Check :
            {
                ...
                oManager.ReleventUpdateForSDLOrWL(new Guid(ug), score, pfScore, fpfScores, rvid, cid, verityId, verityScore, geoCorrelationFlag, verityDOBFail, rdjson);
            }
            else
            {
