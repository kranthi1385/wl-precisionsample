...
define(['app'], function (app) {
    ...
    function ($rootScope, $scope, $timeout, $window, httpService, translationsLoadingService, $cookies) {
            ...
            if (valid) {
                ...
                httpService.postData('/Ep/saveUser', user).then(function (response) {
                    ...
                    if (response != "") {
                        ...
                        $scope.changeClick = false;
                        $scope.getUserDetails();
                        $scope.SaveSettingsMsg = true;
                    }
                    else
                    {
                        $scope.SaveSettingsMsg = false;
                    }
                }, function (err) {
                    $scope.errMsg = true;
                });   
        ...
        $scope.Updatepassword = function () {
            $('#modal_popup1').modal({ backdrop: 'static' });
        }
        $scope.DoNotSellInfo = {
            OldPassword: "",
            NewPassword: "",
        ...
        $scope.changePassword = function (valid) {
            if (valid) {
                ...
                $scope.DoNotSellInfo.NewPassword = encodeURIComponent($scope.DoNotSellInfo.NewPassword);
                $scope.DoNotSellInfo.OldPassword = encodeURIComponent($scope.DoNotSellInfo.OldPassword);
                $scope.DoNotSellInfo.CnfNewPassword = encodeURIComponent($scope.DoNotSellInfo.CnfNewPassword);
                        ...
                        if ($scope.DoNotSellInfo.NewPassword != $scope.DoNotSellInfo.OldPassword) {
                            if ($scope.captchaResponse != '') {
                                httpService.postData('/Common/ValidateCaptcha?googleResponse=' + $scope.captchaResponse).then(function (response) {
                                    if (response == 1) {
                                        httpService.postData('/Login/ChangePassword?OldPassword=' + $scope.DoNotSellInfo.OldPassword + '&NewPassword=' + $scope.DoNotSellInfo.NewPassword + '&CnfNewPassword=' + $scope.DoNotSellInfo.CnfNewPassword + '&ug=' + $scope.user.UserGuid).then(function (response) {
                                            if (response == 1) {
