...
namespace Members.OpinionBar.Web.Controllers
{
    ...
    public class MrController : BaseController
    {
        ...
        [HttpGet]
        public int GetRewardAccessPwd(string password)
        {
            int result = 0;
            UserManager oManager = new UserManager();
            return result = oManager.GetRewardAccessPwd(Identity.Current.UserData.UserGuid, password);
        }
        ...
        public JsonResult GetRewardData(int UserId, Guid UserGuid, int ClientId, string AccBalance)
        {
                            ...
                            if (Convert.ToString(InnerXN["valueType"].InnerText) == "VARIABLE_VALUE" || Convert.ToString(InnerXN["valueType"].InnerText) == "FIXED_VALUE")
                            {
                                ...
                                if (objReward.Sku != "U016495" && objReward.Sku != "U556729" && objReward.Sku != "U569744" && objReward.Sku != "U798603" && objReward.Sku != "U624418")
                                {
                                    ...
                                    if (Convert.ToString(InnerXN["valueType"].InnerText) == "VARIABLE_VALUE")
                                    {
                                        ...
                                        //if ((objReward.MinPrice) < CurrentAcBalance && objReward.MaxPrice >= 1000 && CurrentAcBalance >= 1000)
                                        //{
                                        //    if ((objReward.MinPrice) <= 100)
                                        //    {
                                        //        Range objRange = new Range();
                                        //        objRange.RewardValue = 100;
                                        //        LstRange.Add(objRange);

                                        //        Range objRange1 = new Range();
                                        //        objRange1.RewardValue = 200;
                                        //        LstRange.Add(objRange1);

                                        //        Range objRange2 = new Range();
                                        //        objRange2.RewardValue = 400;
                                        //        LstRange.Add(objRange2);

                                        //        Range objRange3 = new Range();
                                        //        objRange3.RewardValue = 1000;
                                        //        LstRange.Add(objRange3);
                                        //    }
                                        //    else if ((objReward.MinPrice) <= 500)
                                        //    {
                                        //        Range objRange1 = new Range();
                                        //        objRange1.RewardValue = 500;
                                        //        LstRange.Add(objRange1);

                                        //        Range objRange2 = new Range();
                                        //        objRange2.RewardValue = 1000;
                                        //        LstRange.Add(objRange2);
                                        //    }
                                        //}
                                        //else if ((objReward.MinPrice) < CurrentAcBalance && objReward.MaxPrice >= 500 && CurrentAcBalance >= 500)
                                        //{
                                        //    if ((objReward.MinPrice) <= 100)
                                        //    {
                                        //        Range objRange = new Range();
                                        ...
                                        //    }
                                        //}
                                        //else if ((objReward.MinPrice) < CurrentAcBalance && objReward.MaxPrice > 1000 && CurrentAcBalance >= 1000)
                                        //{
                                        //    if ((objReward.MinPrice) <= 1000)
                                        //    {
                                        //        Range objRange = new Range();
                                        //        objRange.RewardValue = 100;
                                        //        LstRange.Add(objRange);
            ...
            objUser.Country_id = (objUser.Country_id == "UK" ? "GB" : objUser.Country_id);
        ...
        [HttpGet]
        [ValidateJsonAntiForgeryToken]
        public ActionResult rc(string lc)
        ...
        public JsonResult RedeemMemberRewards(string Sku, decimal Ut, int Points, int cid, string name)
        {
            ...
            logger.Info("RewardReemption| " + HttpContext.Request.Headers["X-Forwarded-For"].ToString() + "| User GUID: " + Identity.Current.UserData.UserGuid + "| URL: " + Request.Url.AbsoluteUri);
        ...
        public JsonResult GetRewardDetails(string sku, int cid, string name)
        {
            ...
            //objTrewards.LocalCurrency = objRewards.LocalCurrency;
            //objTrewards.CurrencyNotation = objRewards.CurrencyNotation;
        ...
        public JsonResult InserrtRewardRedeemprtions(string amount, string cg)
        {
            ...
            IpCheck = HttpContext.Request.Headers["X-Forwarded-For"].ToString();
            logger.Info("RewardReemption| " + HttpContext.Request.Headers["X-Forwarded-For"].ToString() + "| User GUID: " + Identity.Current.UserData.UserGuid + "| URL: " + Request.Url.AbsoluteUri);
            // Validate that the amount is a multiple of 20
            int amountValue;
            if (!int.TryParse(amount, out amountValue) || amountValue < 20 || amountValue % 20 != 0)
            {
                return Json(new { error = "Amount must be a multiple of 20" }, JsonRequestBehavior.AllowGet);
            }
            if (!string.IsNullOrEmpty(IpCheck))
            {
                ipaddress = IpCheck.Split(',');
                int redeemId = obj.InserrtRewardRedeemprtions(Convert.ToInt32(amount), new Guid(cg), MemberIdentity.Client.ClientId, Identity.Current.UserData.UserGuid, ipaddress[0]);
                return Json(redeemId, JsonRequestBehavior.AllowGet);
            }
            return Json(new { error = "IP address not found" }, JsonRequestBehavior.AllowGet);
