...
namespace Members.PrecisionSample.Components.Data_Layer
{
    public class RewardDataServices
    {
        ...
        public string GetFBCodesForUser(int redemption_id)
        {
            ...
            SqlConnection cn = new SqlConnection();
            SqlCommand cm = new SqlCommand("[rms].[GetFBCodesForUser]", cn);
            cn.Open();
            cm.Parameters.AddWithValue("redemption_id", redemption_id);
            cm.Parameters.AddWithValue("@org_id", MemberIdentity.Client.ClientId);
            using (IDataReader reader = cm.ExecuteReader())
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            codes = codes + Convert.ToString(reader["code_name"]);
                        }
        ...
        public Rewards GetRedeemHistory(string UserGuid)
        {
            ...
            try
            {
                ...
                using (SqlDataReader reader = cm.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        RewardsRedeemedHistory oRewardsRedeemedHistory = new RewardsRedeemedHistory();
                        if (reader["redemption_amount"] != DBNull.Value)
                        {
        ...
        public void RedeemMemberRewards(string Sku, decimal Ut, int UserId, int Points, int OrgId, string FirstName, string EmailAddress, Guid ug, string Ip)
        {
