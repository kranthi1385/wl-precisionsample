...
function ($scope, $http, $window, $location, $rootScope, $timeout, httpService, translationsLoadingService, vcRecaptchaService, getQueryParams, $cookies) {
    ...
    $scope.userLoginClick = function (valid) {
        ...
        window.location.href = 'https://opinionbar.com/Ms/Surveys';
        ...
        if (valid) {
            ...
            httpService.postData('/Home/ObLogIn', $scope.loginDetails).then(function (response) {
                if (response != "") {
    ...
    var registration = function () {
        httpService.postData('/Home/ObsaveUser', $scope.user).then(function (res) {
