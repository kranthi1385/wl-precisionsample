...
namespace Members.PrecisionSample.Web.Controllers
{
    ...
    public class MrController : BaseController
    {
        ...
        public ActionResult Rewards()
        {
            ...
            return View("/Views/Render/Rewards.cshtml");
        }
        ...
        {
            return View("/Views/Render/RewardRedeem.cshtml");
        }
        #endregion
        ...
        [HttpGet]
        [ValidateJsonAntiForgeryToken]
        public JsonResult RedeemMemberRewards(string Sku, decimal Ut, int Points, int cid)
        {
            RewardManager objRewardManager = new RewardManager();
            ...
            if (!string.IsNullOrEmpty(IpCheck))
            {
                ...
                objRewardManager.RedeemMemberRewards(Sku, Ut, objMemberEntity.UserId, Points, objMemberEntity.OrgId, objMemberEntity.FirstName, objMemberEntity.EmailAddress, new Guid(Identity.Current.UserData.UserGuid), ipaddress[0]);
            }
        ...
        public JsonResult InserrtRewardRedeemprtions(string amount, string cg)
        {
            ...
            int amountValue;
            if (!int.TryParse(amount, out amountValue) || amountValue % 20 != 0)
            {
                return Json(new { error = "Amount must be a multiple of 20" }, JsonRequestBehavior.AllowGet);
            }

            // Proceed with normal processing if amount is valid
            if (!string.IsNullOrEmpty(IpCheck))
            {
                ipaddress = IpCheck.Split(',');
                int redeemId = obj.InserrtRewardRedeemprtions(amountValue, new Guid(cg), MemberIdentity.Client.ClientId, Identity.Current.UserData.UserGuid, ipaddress[0]);
