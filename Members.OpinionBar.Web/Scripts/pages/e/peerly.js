...
angular.module("peerlyApp", ['pascalprecht.translate', 'customSerivces', 'vcRecaptcha'])
   ...
   .controller("peerlyregCtrl", ['$scope', '$http', '$window', '$location', '$rootScope', '$timeout', 'httpService', 'translationsLoadingService', 'vcRecaptchaService', 'getQueryParams',
            ...
            if ($scope.isZiprMsg == false) {
                ...
                httpService.postData('/cor/extuserInsert', $scope.user).then(function (res) {
                    if (res != "" && res != undefined && res != "00000000-0000-0000-0000-000000000000") {
                        ...
                        if ($scope.prjid > 0) {
                            window.location.href = 'https://widget.reachcollective.com/survey/entry?mid=' + res + '&isp2=t&project=' + $scope.prjid + '&key=' + $scope.authkey;
                        }
                        else {
                            window.location.href = '/cor/surveyget?userGuid=' + res + '&authkey=' + $scope.authkey + '&isp2=t&prjid=' + $scope.prjid;
                        }
                        //window.location.href = '/cor/p2?ug=' + res + "&pg=" + $scope.authkey + "&cid=" + cid;
                    }
                    else {
