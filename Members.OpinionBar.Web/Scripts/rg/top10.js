...
var LoadBasicQtns = function () {
    ...
    var SurveyQuestion = function (questions) {
        ...
        Self.Skip = function () {
            if (uig != "" & uig != undefined) {
                //$.ajax({
                //    url: '/Misc/ProfileService.aspx?Mode=Inserttop10skiplog&usid=' + ug + '&uig=' + uig,
                //    headers: {
                //        '__RequestVerificationToken': document.getElementsByName("__RequestVerificationToken")[0].value
                //    },
                //    type: 'POST',
                ...
                //        $('.preloader').hide();
                if (usg.toLowerCase() == "6AC169C6-DF47-4CD1-8F4D-1311F5C5F163".toLowerCase()) { //member completed survey success status
                    window.location.href = "/mem/ms?ug=" + ug + "&uig=" + uig + "&usg=" + usg;
                }
                else {
                    window.location.href = "https://e.reachcollective.com/e/tas?ug=" + ug + "&usg=" + s + "&uig=" + uig;
                }
                //    }
                //});
                ...
                //window.location.href = '/rg/OptPage3?ug=' + ug + '&lpage=' + lpage + '&confirm=f';
                window.location.href = '/Ms/Surveys?ug=' + ug;
            }
        }
        Self.Save = function () {
            //$("#btnSubmit").hide();
    ...
    function SaveOptions() {
        ...
        if (Validation()) {
            ...
            if (xml.indexOf("question_id") != -1) {
                $.ajax({
                    url: '/Rg/Top10Save?ug=' + ug + '&rstext=' + _restext + '&rq1=' + _rq1 + '&rq2=' + _rq2 + '&rq3=' + _rq3 + '&rq4=' + _rq4 + '&rg=' + RegStep + '&cid=' + cid,
                    type: 'POST',
                    data: {
                        xml: xml
                    },
    ...
    var redirectUrls = function () {
        if (uig != '') { //user invitataion present
            //is non standalone partner redirect to take another survey page
            if (orgInfo[2] == "0" && (usg.toLowerCase() == "f6a6b754-4cf8-41bb-b9aa-5b97c412b1f4".toLowerCase() ||
                usg.toLowerCase() == "67b98bed-9c3f-42ae-bdd3-7e15f9c17f00".toLowerCase() ||
                usg.toLowerCase() == "d5f04cf6-50ab-4617-9b0f-95b23a07488c".toLowerCase())) {
                window.location.href = 'https://e.reachcollective.com/e/tas?ug=' + ug + '&uig=' + uig + '&usg=' + usg + '&cid=' + cid;
            }
            else {
                //standalone partner  redirect to 
                if (parseInt(orgInfo[3]) == -1 || parseInt(orgInfo[3]) == 1 || parseInt(orgInfo[3]) == 3) {
                    window.location.href = orgInfo[0] + '/profile/surveys?ug=' + ug + '&uig=' + uig + '&usg=' + usg + '&cid=' + cid;
                }
                else {
                    window.location.href = orgInfo[0] + '/partner/end?ug=' + ug + '&uig=' + uig + '&usg=' + usg + '&cid=' + cid;
                }
            }
        }
        else {
            if (cid == 337 || cid == 491 || cid == 38) {
                window.location.href = '/Ms/Surveys?ug=' + ug;
            }
            else {
                //window.location.href = '/rg/OptPage3?ug=' + ug + '&lpage=' + lpage + '&confirm=f' + '&cid=' + cid;
                window.location.href = '/Ms/Surveys?ug=' + ug;
            }
        }
    }
