    ...
    function ($rootScope, $scope, $window, translationsLoadingService, httpService, $cookies, pagerService, getCurrentPageList) {
        ...
        httpService.getData('/ep/GetUserData').then(function (response) {
            ...
            httpService.postData('/mr/GetRewardsHistory', "").then(function (response) {
                ...
                //$scope.userRewardsInfo.LocalCurrency = Math.round($scope.userRewardsInfo.LocalCurrency);
                //// to show currency based on user country
                //if ($scope.userRewardsInfo.CurrencyNotation == "") {
                //    $scope.userRewardsInfo.CurrencyNotation = $scope.userRewardsInfo.RewardText;
                //}
                //$scope.RewardText = $scope.userRewardsInfo.CurrencyNotation;
                if (response == '') {
        ...
        $scope.RewardClick = function (parent, obj, $event) {
          ...
          exp = new $window.Date(now.getFullYear(), now.getMonth() + 6, now.getDate());
            ...
            $cookies.put('IsFraud', $scope.user.IsFraud, {
                expires: exp,
                path: '/'
            });
        ...
        function skustoredinsession(parent, event, id, rname) {
            ...
            if (rname == 'd') {
                ...
                window.location.href = "/mr/rc?img=" + encodeURIComponent(parent.ImageURL) + "&id=" + id + "&name=" + encodeURIComponent(parent.Description) + '&des=' + encodeURIComponent(parent.Description) + '&rn=' + encodeURIComponent(parent.Description) + '&cg=' + parent.CatalougeGuid + '&cid=' + $scope.user.OrgId + '&countryId=' + $scope.user.CountryId + '&lc=' + $scope.recaptchalangCode;
            }
            else {
                var rewvalue = parseInt(event.RewardValue * 9876);
                window.location.href = "/mr/rc?img=" + encodeURIComponent(parent.ImageURL) + "&id=" + id + '&name=' + encodeURIComponent(parent.Description) + '&des=' + encodeURIComponent(parent.Description) + '&cg=' + parent.CatalougeGuid + '&cid=' + $scope.user.OrgId + '&countryId=' + $scope.user.CountryId + '&lc=' + $scope.recaptchalangCode;
            }
            //}, function (err) {
            //});
        }
