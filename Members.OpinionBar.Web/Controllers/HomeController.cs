...
namespace Members.OpinionBar.Web.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        #region RewardLogin Action method
        /// <summary>
        /// RewardLogin Action Method
        /// </summary>
        /// <returns></returns>
        // GET: Home
        //public ActionResult RewardLogin()
        //{ 
        public ActionResult RewardsLogin(string ug, string rid, string sid, string txid, string trans_id)
        {
            ...
            logger.Trace($"RewardsLogin Method : IP : {ipAddress} | GUID : {ug} | RID : {rid} | SID : {sid} | TXID : {txid} | TRANSID {trans_id} ");
            if (rid != string.Empty)
            {
                ReferrerIds = (rid + "/" + sid + "/" + txid + "/" + trans_id + "///").Split('/');
                int clickId = oUserManager.InserClicks(ReferrerIds[0], ReferrerIds[1], ReferrerIds[2], IpAddress, redirectUrl, 1, 0, 0);
            }
            ...
            else
            {
                ...
                User oUser = new Components.Entities.User();
                ViewBag.IsShowLogIn = 1;
                return View("~/Views/Account/RewardsOBLogin.cshtml", oUser);
            }

        }
        #endregion

        #region Rewards Signup
        public ActionResult RewardsSignup()
        {
            ViewBag.SelectedMenu = "RewardsSignup";
            return View("~/Views/Home/RewardsOBSignup.cshtml");
        }
        #endregion
