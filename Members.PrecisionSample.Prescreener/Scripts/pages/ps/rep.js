    ...
    psApp.register.controller('messageController', ['$rootScope', '$scope', '$http', '$window', 'httpService', 'getQueryParams', 'translationsLoadingService',
        function ($rootScope, $scope, $http, $window, httpService, getQueryParams, translationsLoadingService) {
            ...
            if (ug == uig) {
                ...
                httpService.getData(url).then(function (response) {
                    if (response.ProjectId > 0 && response.ProjectId != null && response.ProjectId != "") {
                        ...
                        $window.location.href = "https://e.reachcollective.com/e/psr?usg=2B9038B6-DB53-429A-8854-7BB83338B2D4&ug=" + ug + '&cid=' + cid + "&uig=" + $scope.UserInvitationGuid + '&pid=' + pid;
                    }
                });
            ...
            $scope.successClick = function () {
                ...
                //Based on the 
                window.location.href = "https://e.reachcollective.com/e/interstitial?ug=" + ug + "&uig=" + uig + "&sr=" + $scope.enableRecaptcha + "&cid=" + cid + '&fc=n&pid=' + pid;
            }
            $scope.noThanks = function () {
                $window.location.href = "https://e.reachcollective.com/e/psr?usg=FA27DD1D-CA1D-478B-A66A-93C589361D34&ug=" + ug + '&cid=' + cid + "&uig=" + $scope.UserInvitationGuid + '&pid=' + pid;
            }
        }])
});