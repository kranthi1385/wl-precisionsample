...
namespace Members.PrecisionSample.Prescreener.Controllers
{
    public class PrsController : Controller
    {
        ...
        [ValidateJsonAntiForgeryToken]
        [HttpGet]
        [Route("SavePrescreenerLanguage")]
        public string SavePrescreenerLanguage(Guid ug, Guid uig, int cid, int langid)
        {
            return oManager.SavePrescreenerLanguage(ug, cid, uig, langid);
        }
        #endregion
        ...
        public JsonResult vldEmail(string email)
        {
            ...
            try
            {
                ...
                logger.Debug("XverifyCall|" + Request.Url.AbsoluteUri.ToString() + "| Domain :" + ConfigurationManager.AppSettings["Xverifydomain"].ToString() + "| API :" + url + "| response :" + errormessage);
