...
namespace Members.PrecisionSample.Components.Business_Layer
{
    public class UserManager
    {
        ...
        /// <param name="RelevantId"></param>
        /// <returns></returns>
        public int ReleventUpdate(Guid UserGuid, int RelevantScore, int PfScore, string FpfScore, string RelevantId, int ClientId, string userId, string sessionId)
        {
            UserDataServices oUserDataServices = new UserDataServices();
            string sessionResponse = string.Empty;
            if (sessionId != "undefined")
            {
                sessionResponse = AuthenticateSessionId(sessionId, userId.ToString(), null);
            }
            return oUserDataServices.ReleventUpdate(UserGuid, RelevantScore, PfScore, FpfScore, RelevantId, ClientId, sessionResponse);
        }
        #endregion

        #region Verisoul Authentication
        private string AuthenticateSessionId(string sessionId, string userId, string pid)
        {
            string sessionResponse = string.Empty;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(verisoulURL) && !string.IsNullOrEmpty(verisoulAPIKey))
                {
                    ...
                    var body = new
                    {
                        ...
                        account = new { id = $"{userId}", group = "Reach_Collective" }
                    }; 
                    ...
                    if(!string.IsNullOrEmpty(pid))
                    {
                        ...
                        {
                            session_id = sessionId,
                            account = new { id = $"{userId}-{pid}", group = pid.ToString() }
                        };
