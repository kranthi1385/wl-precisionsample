...
namespace Members.PrecisionSample.Components.Business_Layer
{
    public class ProfileQuestionBusinessService
    {
        ...
        /// <param name="Ug"></param>
        /// <returns></returns>
        public PiiResponse GetQuestion(Guid Uig, Guid Ug, string ProjectId, string TargetId, string UserId, string cid, string clientIp,string dvtype)
        {
            PiiRequest request = new PiiRequest()
            {
                ...
                ProjectId = string.IsNullOrEmpty(ProjectId) ? 0 : Convert.ToInt32(ProjectId),
                TargetId = string.IsNullOrEmpty(TargetId) ? 0 : Convert.ToInt32(TargetId),
                DeviceType = dvtype,
